...
namespace DemoClient.View.LRN
{
    ...
    /// </summary>
    public partial class LRN0110 : DemoClient.Controllers.BasePopupForm
    {
        // 검색 쓰레드
        private Thread _thread;
        private DataTable ReturnData { get; set; }

        #region LRN0110 : 생성자 함수
        ...
        #region LRN0110_Load : 폼 로드 이벤트
        private void LRN0110_Load(object sender, EventArgs e)
        {
            try
            {   
                ...
                _dtpEXECDT.Value = DateTime.Now;
                _dtpLNSTDT.Value = DateTime.Now.AddDays(1);
        ...
        int Search()
        {
            ...
            try
            {
                ...
                DataTable _dt = base.GetDataTable("PCSP_LRN0111_R1"
        ...
        {   
            try
            {
                base.ExecuteNonQuery("PCSP_LRN0110_C1"
                    , _txtSTR_CD.Text					    // 가맹점코드
                    , _cmbLNR_CD.SelectedValue          	// 여신사코드
                    , base.GetInteger(_txtLNAMT)	    	// 대출신청금액
                    , base.GetInteger(_txtLNMNT)			// 대출일수
                    , base.GetDecimal(_txtINTRRTYEAR)		// 대출연이율
                    , _dtpEXECDT.Value      				// 대출일자
                    , _dtpEXECDT.Value.AddDays(1)			// 상환일자
                    , _txtBIGO.Text                         // 비고
                    , base.GetCookie("USRID")				// 시스템수정자ID
                    , base.GetCookie("USRNM")				// 시스템수정자명
                    );

                MessageBox.Show("비즈론 대출원장을 등록 하였습니다.");
